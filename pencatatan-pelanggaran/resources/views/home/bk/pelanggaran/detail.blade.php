                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <td colspan="2" class="text-center bg-warning text-white">
                                                        {{ $total_poin > 100 ? '100 (Max)' : $total_poin }}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                            ...
                            <form action="{{ route('pelanggaran.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col">
                                        <div class="form-group">
                                            <label>Pelapor</label>
                                            <input type="text" class="form-control"
                                                value="{{ optional($pelanggaran->User)->nama ?? optional($pelanggaran->Petugas_Bk)->nama }}"
                                                readonly>
                                        </div>
                                    </div>
                                    @if ($pelanggaran->id_bk)
                                        <div class="col">
                                            <div class="form-group">
                                                <label>Terakhir Diubah</label>
                                                <input type="text" class="form-control"
                                                    value="{{ $pelanggaran->Bk->nama }}" readonly>
                                            </div>
                                        </div>
                                    @endif
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label>Hukuman</label>
                                            <input type="text"
                                                value="{{ $pelanggaran->Hukuman_Pilihan->Hukuman->hukuman ?? 'Belum Di Proses' }}"
                                ...
                                <div class="modal-footer">
                                    @if ($pelanggaran->status == 'Belum')
                                        <a href="{{ route('review.review', $pelanggaran->id) }}" class="btn btn-info"><i
                                                class="fa fa-edit mr-2"></i>Proses Pelanggaran</a>
                                    @else
                                        @if ($pelanggaran->is_active == 1)
                                            <a href="{{ route('review.edit', $pelanggaran->id) }}" class="btn btn-info"><i
                                                    class="fa fa-edit mr-2"></i>Edit Pelanggaran</a>
                                        @endif
                                        @if ($pelanggaran->is_active == 0)
                                            <a href=""
                                                class="btn d-flex align-items-center btn-warning btn-go-public">
                                        ...
                                        @else
                                            <a href=""
                                                class="btn d-flex align-items-center btn-success btn-go-private">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 24 24">
                                                    <path fill="white"
                                    ...
                                    @endif
                                    @if ($pelanggaran->status == 'Belum')
                                        <a onclick="confirmDel('{{ route('review.destroy', $pelanggaran->id) }}')"
                                            class="btn btn-danger" data-toggle="tooltip" title="Hapus"><i
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.btn-go-public').click(function(e) {
                e.preventDefault();
                swalConfirm('Konfirmasi Tampilkan Pelanggaran!', 'Data Akan Ditampilkan', 'warning',
                    "{{ route('review.public', $pelanggaran->id) }}")
            })

            $('.btn-go-private').click(function(e) {
                e.preventDefault();
                swalConfirm('Konfirmasi Sembunyikan Pelanggaran!', 'Data Akan Disembunyikan', 'warning',
                    "{{ route('review.private', $pelanggaran->id) }}")
            })
        })
    </script>
@endsection
